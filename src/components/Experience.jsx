          ...
          {experiences.map((exp, index) => (
            ...
            >
              <div className="hidden md:flex flex-col items-center pt-2">
                <div className="w-5 h-5 rounded-full bg-gradient-to-br from-accent to-cyan-400 ring-4 ring-accent/20 shadow-card-hover shrink-0" />
              ...
              <div className="bg-surface border border-border hover:border-muted transition-colors rounded-xl p-8 md:p-10 md:col-start-2">
                <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
                  <div className="flex items-start gap-4">
                    <div className="p-3.5 rounded-xl bg-gradient-to-br from-accent/20 to-accent-purple/10 border border-accent/25 shadow-card-hover">
                      <HiBriefcase className="text-foreground" size={24} />
                    </div>
                    <div>
                      <h3 className="font-display text-xl md:text-2xl font-bold text-foreground tracking-tight">
                        {exp.company}
                      </h3>
                      <p className="text-foreground font-semibold mt-1">{exp.role}</p>
                    </div>
                  </div>
                  <span className="tag-pill text-foreground border-accent/30 bg-foreground/5 font-bold">
                    {exp.period}
                  </span>
                </div>
                ...
                <div className="flex flex-wrap gap-2.5">
                  {exp.highlights.map((tag) => (
                    <span
                      key={tag}
                      className="px-3.5 py-2 rounded-lg text-xs font-bold text-foreground bg-foreground/10 border border-accent/25 hover:bg-foreground/20 hover:shadow-card-hover transition-all duration-300"
                    >
                      {tag}
                    </span>
                  ))}
